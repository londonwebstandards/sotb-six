...
const CACHES = [STATIC_CACHE, ASSETS_CACHE, IMAGES_CACHE, PAGES_CACHE];
...
function clearOldCaches() {
  return caches.keys().then(keys => {
    return Promise.all(
      keys.filter(key => !CACHES.includes(key)).map(key => caches.delete(key))
    );
  });
}
      ...
      fetch(request)
        .then(response => {
          ...
          if (
            ...
            !CRITICAL_FILES.includes(url.pathname) &&
            !CRITICAL_FILES.includes(url.pathname.slice(1))
          ) {
